...
### Conclusion
The MSE from the train and test datasets were expectedly high with the test MSE and train MSE being very close to each other for both features. Throughout all 10 folds of Cross Validation, Material Advantage MSE ranged between roughly 370 to 375, where test and train can both either be the lower one. The same applies to Development MSE but with an expectedly higher range of roughly 403 to 412. <br> 
After viewing the graphical data and MSE, we have concluded that not only is the linear regression model not ideal, but trying to minimize MSE for this type of problem is very unlikely to work due to erractic placements of our datapoints. We also found it difficult to fully utilize linear regression modeling with our independent variables since getting different linear functions does not really help much with the goal of predicting position evaluations. <br>
...
### Training the Model
We used the Keras Sequential modeling for our Neural Network, which has 7 Dense hidden layers with varying number of units (ranging from 64 to 512 due to the large number of datapoints our dataset contained), all using the *RELU* activation function. Our output layer uses the *softmax* activation function due to this being a multi-classification problem. We ran the fit with a batch size of 8 and 5 epochs, which we felt was a good tradeoff between training speed while maintaing some degree of accuracy. Our first/official run of the Neural Network consisted of a 90:10 train-test split. When doing validation, we did a 5-fold Cross Validation, which resulted in an 80:20 train-test split. <br>
...
To validate our model, we did a 5-fold Cross Validation which resulted in an 80:20 train-test split so we can have more test data to predict while also saving time in comparison to a 10-fold Cross Validation.

### Conclusion
The train and test accuracy are roughly similar, so we at least know that our model is not overfit, and during the 5-fold Cross Validation, accuracy stayed the same for each iteration. As we can see from the classification report and confusion matrix though, our model was not very accurate at predicting the positional advantages of chess positions given the features we were able to extract from the FEN string. However, we do feel that this was the best the model can do given that there weren't really any strong correlations between any of the features, including the dependent variable. We believe that this only further proves how difficult it is to judge chess positions and that even to this day, Chess is still an NP-Hard problem that has many chess engines and programmers that are still continuing to develop heuristics and algorithms to improve their gameplay. 

#### Where does your model fit in the fitting graph?
Due to the complexity of a neural network, this model certainly fits near the middle of the fitting graph. It has enough complexity that it would have lower predictive error, but given that the neural network we intend to create is not overly deep, it should not overfit and would prevent a significant difference in test error vs training error.
