...
### Training the Model
We used the Keras Sequential modeling for our Neural Network and had 7 Dense hidden layers with varying number of units (ranging from 64 to 512 due to the large number of datapoints our dataset contained) and all using the *RELU* activation function. Our output layer uses the *softmax* activation function due to this being a multi-classification problem. We ran the fit with a batch size of 8 and 5 epochs, which we felt was a good tradeoff between training speed while maintaing some degree of accuracy. We also did a train-test split of 90:10 when doing the "official" run of our model. <br>
